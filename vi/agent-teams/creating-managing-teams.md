...
Trang danh sách Teams hỗ trợ **toggle card/list** để chuyển đổi giữa giao diện thẻ trực quan và dạng danh sách gọn.
...
Thông tin member trả về từ API được làm giàu với đầy đủ **metadata agent** (display name, emoji, mô tả, model) để dashboard có thể hiển thị thẻ member phong phú.
...
Khi team đang hoạt động, GoClaw tự động inject phần `## Team Members` vào system prompt của lead agent, liệt kê tất cả các teammate. Mỗi mục được làm giàu với metadata agent bao gồm biểu tượng emoji (từ `other_config`):
...
## Giải quyết Workspace của Lead

Khi một team task được dispatch, lead agent giải quyết thư mục workspace theo từng team cho cả lead lẫn member. Việc giải quyết này hoàn toàn trong suốt — agent dùng đường dẫn file thông thường và **WorkspaceInterceptor** tự động chuyển hướng yêu cầu đến đúng context workspace team.

Với scope isolated (`workspace_scope: "isolated"`), mỗi cuộc hội thoại có thư mục riêng. Với scope shared, tất cả member đọc và ghi vào cùng thư mục team.
