...
| `agent` | Toàn bộ run lifecycle, output preview |
| `embedding` | Tạo embedding cho vector store |
| `event` | Marker sự kiện rời rạc (không có duration) |
...
Traces UI bao gồm:
- **Timestamp** trên mỗi span để timing chính xác
- **Nút copy** trong span details để dễ dàng export dữ liệu trace
- **Syntax highlighting** trên JSON payload trong span preview
...
Ở verbose mode, LLM span lưu đầy đủ input/output tối đa 200 KB; tool span lưu đầy đủ input và output tối đa 200 KB.

> Chỉ dùng verbose mode ở môi trường dev — full message có thể rất lớn.

## Export Trace

Trace riêng lẻ (bao gồm tất cả span và sub-trace) có thể export qua HTTP:

```
GET /v1/traces/{traceID}/export
```

Response là **JSON nén gzip** chứa trace, các span của nó, và child trace được thu thập đệ quy (`sub_traces`). Hữu ích để phân tích offline, báo cáo lỗi, hoặc lưu trữ các agent run dài.
...
OTel exporter chỉ được compile khi thêm `-tags otel`. Build mặc định không có dependency OTel, tiết kiệm khoảng 15–20 MB dung lượng binary.
...
OTel exporter batch spans với max batch size 100 và timeout 5 giây.
