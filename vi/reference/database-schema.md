...
Phiên bản schema được theo dõi bởi `golang-migrate`. Chạy `goclaw migrate up` hoặc `goclaw upgrade` để áp dụng tất cả migration. Phiên bản schema hiện tại: **49**.
...
**Index:** `tenant_id`, `(agent_id, scope)`, `(agent_id, doc_type)`, `content_hash`, HNSW cosine trên `embedding`, GIN trên `tsv`, `team_id` (partial non-null), `idx_vault_docs_path_prefix` trên `(path text_pattern_ops)` (migration 049)
