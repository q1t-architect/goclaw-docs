...
Phiên bản schema được theo dõi bởi `golang-migrate`. Chạy `goclaw migrate up` hoặc `goclaw upgrade` để áp dụng tất cả migration. Phiên bản schema hiện tại: **44**.
...
Registry tài liệu Knowledge Vault. Filesystem chứa nội dung; DB chứa path, hash, embedding và link. (migration 038; cột `summary` thêm ở migration 042; `team_id`, `custom_scope` thêm ở migration 043)
...
**Index:** `tenant_id`, `(agent_id, scope)`, `(agent_id, doc_type)`, `content_hash`, HNSW cosine trên `embedding`, GIN trên `tsv`, `team_id` (partial non-null)
...
Liên kết hai chiều kiểu wikilink giữa các tài liệu vault. (migration 038; `custom_scope` thêm ở migration 043)
...
| `link_type` | TEXT | NOT NULL DEFAULT `wikilink` | `wikilink` hoặc `reference` |
