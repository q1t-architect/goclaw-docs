...
GoClaw phát hiện vòng lặp tool: **cảnh báo** được ghi sau 3 lần gọi giống nhau liên tiếp, và vòng lặp bị **dừng bắt buộc** sau 5 lần gọi giống nhau không có tiến triển. Lưu ý: các tool `exec`/`bash` và MCP bridge tool (tiền tố `mcp_*`) được xem là **trung lập** — chúng không reset cũng không tăng chuỗi read-only, vì side effect của chúng không xác định.
...
| **Tool Registry** | 50+ tool tích hợp sẵn với kiểm soát truy cập dựa trên policy (mở rộng qua MCP và custom tool) |
