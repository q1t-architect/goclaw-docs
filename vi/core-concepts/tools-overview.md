...
|----------|-------|----------|
| **Filesystem** (`group:fs`) | read_file, write_file, edit, list_files, search, glob, send_file | Đọc, ghi, chỉnh sửa và tìm kiếm file trong workspace của agent; `send_file` deliver file đã tồn tại dưới dạng attachment |
...
Ngoài ghi đè per-agent, admin có thể bật/tắt deny group **toàn cục** qua `config.tools.shellDenyGroups` (`map[string]bool`):
...
Cấu hình này được **reload tự động khi runtime** qua bus `TopicConfigChanged` — không cần khởi động lại gateway. Per-agent override (`shell_deny_groups` trong cấu hình agent) thắng per-key so với cấu hình toàn cục.

Xem thêm: [deployment/security-hardening](/deployment/security-hardening).
