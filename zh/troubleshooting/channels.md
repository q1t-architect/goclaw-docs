...
| `starting` | Channel 正在启动 |
| `healthy` | 已连接并正在接收消息 |
| `degraded` | 已连接但正在遇到错误 |
| `failed` | Channel 因不可恢复的错误而停止 |
| `stopped` | Channel 已正常关闭 |

当 channel 进入 `failed` 状态时，仪表盘会显示修复提示（如"Review channel credentials"或"Check upstream reachability"）。`failed` 且 `failure_kind: auth` 需要更新凭证——channel 不会自动恢复。`failed` 且 `failure_kind: network` 可重试，channel 会自动重连。
