...
当设置 `mode: "cache-ttl"` 时，裁剪在每次 agent 请求时自动运行。流程：
...
上下文裁剪**默认关闭**。通过在 agent 配置中设置 `mode: "cache-ttl"` 来启用。
...
| 字段 | 默认值 | 描述 |
|-------|---------|-------------|
| `mode` | `"off"` | 设为 `"cache-ttl"` 以启用裁剪。任何其他值均禁用。 |
...
检查 `mode` 是否确实为 `"cache-ttl"`。任何其他值（包括省略该字段）都会禁用裁剪。还要确认 agent 上设置了 `contextWindow` — 裁剪需要 token 数量来计算比率。
