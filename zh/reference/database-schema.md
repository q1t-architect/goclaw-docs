...
Schema 版本由 `golang-migrate` 跟踪。运行 `goclaw migrate up` 或 `goclaw upgrade` 以应用所有迁移。当前 schema 版本：**49**。
...
**索引：** `tenant_id`，`(agent_id, scope)`，`(agent_id, doc_type)`，`content_hash`，`embedding` 上的 HNSW cosine，`tsv` 上的 GIN，`team_id`（partial non-null），`idx_vault_docs_path_prefix` 在 `(path text_pattern_ops)` 上（migration 049）
