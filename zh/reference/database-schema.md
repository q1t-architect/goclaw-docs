...
Schema 版本由 `golang-migrate` 跟踪。运行 `goclaw migrate up` 或 `goclaw upgrade` 以应用所有迁移。当前 schema 版本：**44**。
...
Knowledge Vault 文档注册表。文件系统存储内容；数据库存储路径、哈希、embedding 和链接。（migration 038；`summary` 列在 migration 042 添加；`team_id`、`custom_scope` 在 migration 043 添加）
...
**索引：** `tenant_id`，`(agent_id, scope)`，`(agent_id, doc_type)`，`content_hash`，`embedding` 上的 HNSW cosine，`tsv` 上的 GIN，`team_id`（partial non-null）
...
vault 文档之间的双向 wikilink 样式连接。（migration 038；`custom_scope` 在 migration 043 添加）
...
| `link_type` | TEXT | NOT NULL DEFAULT `wikilink` | `wikilink` 或 `reference` |
